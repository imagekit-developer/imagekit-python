...
from tests.helpers import (
    ClientTestCase,
    create_headers_for_test, get_auth_headers_for_test, make_string_to_single_line,
)
...
class TestUpload(ClientTestCase):
    ...
    def test_binary_upload_succeeds(self):
        ...
        request_body = make_string_to_single_line('''{'customMetadata': '{"test100": 11}',
    ...
    def test_upload_fails_with_400_exception(self) -> None:
        ...
        try:
            responses.add(
                ...
                status=400,
                body='''{
                    "message": "A file with the same name already exists at the exact location.
                     We could not overwrite it because both overwriteFile and useUniqueFileName are set to false."
                }''',
            )
            self.client.upload_file(file=self.image, file_name=self.filename,
                                    options=UploadFileRequestOptions(use_unique_file_name=False, tags=["abc", "def"],
...
class TestDeleteFile(ClientTestCase):
    ...
    def test_bulk_file_delete_succeeds(self):
        ...
        self.assertEqual('{"fileIds": ["fake_123", "fake_222"]}', responses.calls[0].request.body)
        self.assertEqual(camel_dict_to_snake_dict(mock_response_metadata), resp.response_metadata.__dict__)
...
class TestPurgeCache(ClientTestCase):
    ...
    def test_purge_file_cache_succeeds(self):
        ...
        self.assertEqual('{"url": "https://example.com/fakeid/fakeimage.jpg"}',
